 ...
 * 
 * @d2wKey searchKey - either a single target key as a string, an array with
 *         multiple keys or an array of arrays with keys
 * @d2wKey restrictedChoiceKey - key path that will return a list of filter
 *         choices
 * @d2wKey keyWhenRelationship - specifies the display key on the choice
 * @d2wKey restrictedChoiceTargetKey - target key path on which to qualify with
 *         the choice
 * @d2wKey restrictedChoiceRecursionKey - key path from which to retrieve
 *         descendant choices, think "allChildrenCategories"
