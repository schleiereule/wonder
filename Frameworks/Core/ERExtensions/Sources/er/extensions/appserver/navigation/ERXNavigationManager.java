...
public class ERXNavigationManager {
	...
	public String navigationStateFromMap(WOSession session) {
		...
		if (session.valueForKeyPath("objectStore." + ERXNavigationManager.NAVIGATION_MAP_KEY) != null) {
			...
			// the current request's URI serves as the basis for the key
			String keyPath = session.context().request().uri();
			// apply rewrites
			keyPath = ERXApplication.erxApplication()._rewriteURL(keyPath);
			keyPath = removeApplicationURLPrefix(session, keyPath);
			// replace all dots
	...
	public void storeInNavigationMap(WOSession session, ERXNavigationItem navigationItem, String uri) {
		...
		if (uri != null) {
			uri = removeApplicationURLPrefix(session, uri);
	...
	/**
	 * Removes the application URL prefix from the passed URI to avoid issues
	 * with instance IDs in navigation state storage.
	 * 
	 * @param session
	 * @param uri
	 * @return the URI minus the application URL prefix
	 */
	private String removeApplicationURLPrefix(WOSession session, String uri) {
		String appURLPrefix = ERXApplication.erxApplication()._rewriteURL(session.context().request().applicationURLPrefix());
		if (uri.contains(appURLPrefix)) {
			uri = uri.replaceFirst(appURLPrefix, "");
		}
		else {
			// appURLPrefix does not contain an instance number
			String appNumberSuffix = "/" + session.context().request().applicationNumber();
			if (appURLPrefix.endsWith(appNumberSuffix)) {
				appURLPrefix = appURLPrefix.substring(0, appURLPrefix.length() - appNumberSuffix.length());
				uri = uri.replaceFirst(appURLPrefix, "");
			}
		}
		return uri;
